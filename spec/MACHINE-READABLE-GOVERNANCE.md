...
## 6. Decision rules, exact arithmetic and ballot authentication
...
The validator recomputes the electorate from reconstructable Membership history, freezes eligibility at the validated voting-window opening date, verifies recusals, authenticates each counted ballot and recomputes quorum and approval. A stored `result: approved` field is not trusted by itself.

Every counted Member ballot must carry a content-addressed authentication reference. The authenticated payload binds the exact decision ID, decision class, voting-window opening date, Member person ID and vote value. For this draft contract the authentication record must include the Member's signature over that exact payload digest plus verification evidence, and its authentication/signature date must fall inside the voting window. A bare `{person_id, vote}` object or a tally generated by another participant cannot establish that a Member cast a ballot.
...
- a later F0 admission must satisfy the Candidate period, bind a Founding Steward signed admission payload **and prove that the Founding Steward assignment was still operative on that decision date**; and
...
Founding Steward authority is historical state, not merely the current `operative_assignment` flag. The initial assignment must resolve to the exact governance-adoption record and begin on the governance effective date. Any later resignation, succession-triggered cessation or valid removal must resolve to a content-addressed cessation record that binds the exact initial assignment, decision/effective dates, cessation payload and applicable signature/process/approval evidence. Once cessation is effective, editing the projection cannot silently restore the original founding assignment.

Any machine validation of an F0 act that depends on founder authority must therefore reconstruct whether the Founding Steward assignment was active on the **act's decision date**. Remaining in F0 does not preserve authority after a valid cessation.
...
- revocation mechanism and authority;
- immutable creation decision record; and
- when deactivated, a content-addressed revocation decision binding the original grant and exact revocation payload.
...
A signature record used to establish constitutive, admission, ballot, lifecycle, legal-review or CLA authority must identify at least:
