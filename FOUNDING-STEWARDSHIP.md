# Exergism Commons Founding Stewardship

> **Status: 0.1-DRAFT — non-operative.** This document defines a proposed founder-led bootstrap model for Exergism Commons (EC). It does not create legal authority merely by existing in Git.

## 1. Purpose

EC is not yet a mature institution with a broad, independent membership capable of replacing founder-led coordination without substantial continuity risk. Pretending otherwise would hide rather than reduce concentration of power.

The bootstrap model therefore makes current founder authority explicit, bounded and reviewable while preventing that authority from becoming ownership of EC.

The design objective is:

> **strong stewardship without ownership**.

## 2. Founding Steward

The proposed initial **Founding Steward** is **Daniel Molinero Lucas**.

The Founding Steward is a constitutional bootstrap role, not an equity position, transferable property interest, hereditary office or claim on EC assets.

Technical control of GitHub, domains, hosting or repositories is not itself the source of this role. If this framework becomes operative, the role must be recorded in an adopted institutional record.

## 3. Mission Lock

### 3.1 Protected mission

EC exists to steward a durable, contestable and anti-capture commons around Exergism and projects that explicitly depend on, implement or extend that ecosystem.

The Mission Lock protects at least these invariants:

1. EC remains institutionally centered on Exergism rather than being repurposed into an unrelated organization;
2. Exergism remains governed by its own canonical philosophical/formal process rather than by organization-level popularity alone;
3. ECL, ECL-PL, Funding and other projects retain their documented authority boundaries;
4. membership, funding, employment, repository access or technical control do not create economic ownership of EC;
5. no donor or funder purchases organization-wide governance authority merely through money;
6. contributor, copyright and patent rights are not fabricated or retroactively expanded by governance;
7. persistent identifiers and released artifacts are not silently repurposed; and
8. contestability, evidence, reversibility where possible and resistance to capture remain structural governance commitments.

The Mission Lock protects institutional identity. It does **not** freeze every philosophical proposition, implementation choice, roadmap item, ECL criterion or technical architecture. Those may evolve through the authority process of the owning project.

### 3.2 Mission-Locked Amendment

A proposal is a **Mission-Locked Amendment** whenever **any operative effect** of the proposal would alter, weaken, remove, create an exception to, bypass, or materially narrow a Mission Lock invariant.

Classification depends on effect, not title or drafting technique. A protected effect remains mission-locked when it is bundled with unrelated changes, described as secondary or incidental, implemented indirectly through another policy, or presented as part of an ordinary Constitutional Amendment. If reasonable reviewers disagree whether an operative effect reaches a protected invariant, the proposal must use the stricter Mission-Locked Amendment path unless and until the ambiguity is resolved through the applicable governance process.

During the Founding Period, a Mission-Locked Amendment requires all of:

- at least 90% approval among valid for/against votes;
- at least 75% quorum of non-conflicted members eligible for mission-locked voting;
- at least one valid affirmative vote and a non-zero `for + against` denominator in each protected ballot;
- two successful votes separated by at least 60 days;
- a written compatibility and consequences analysis;
- qualified independent review appropriate to the subject; and
- affirmative consent of the Founding Steward or the valid successor Mission Guardian.

Mandatory law prevails where an internal lock cannot lawfully prevent a required change.

## 4. Founder executive authority during bootstrap

While EC is in `founder-led-bootstrap`, the Founding Steward may direct routine institutional formation and strategy, including:

- setting initial organizational roadmap and priorities;
- proposing and maintaining draft policies;
- appointing or removing interim role holders;
- admitting initial Members under the Membership Policy;
- coordinating cross-project infrastructure where no project-specific authority is displaced;
- representing the bootstrap project publicly without falsely claiming nonexistent legal status; and
- taking reversible technical actions reasonably necessary to preserve EC infrastructure.

These powers exist to make EC capable of becoming an institution. They are not unlimited executive sovereignty.

## 5. Founder Mission Veto

During the Founding Period the Founding Steward may block an otherwise valid organization-level proposal only when the proposal reasonably implicates a Mission Lock invariant or an expressly protected irreversible action.

A Mission Veto:

1. is **negative authority only** — it can block a proposal but cannot enact an alternative decision;
2. must identify the exact protected invariant or protected action at issue;
3. must include a written rationale in the decision record;
4. cannot be used to approve the Founding Steward's own compensation, contract, claim, sanction, asset transfer or other conflicted private benefit;
5. cannot create copyright, patent or contractual rights that EC does not possess;
6. cannot rewrite immutable historical releases or decisions; and
7. cannot substitute for a stricter approval process required by mandatory law.

Ordinary disagreement about implementation, taste or policy within the protected mission is not sufficient grounds for Mission Veto once the relevant authority has validly moved to Members or a delegated body.

## 6. No founder economic privilege

Founding status creates **no automatic economic privilege**.

In particular, the Founding Steward has no automatic right to:

- salary or compensation;
- a percentage of grants, donations, revenue, reserves or Endowment returns;
- distribution of Treasury or Endowment assets;
- preferential contracting;
- ownership of organization-wide intellectual property merely because EC was founded by that person; or
- a residual claim on EC assets upon dissolution.

The Founding Steward may be compensated for genuine work under the same lawful basis and conflict rules that apply to any other person. The beneficiary must not substantively approve their own compensation.

## 7. Protected actions the founder cannot take unilaterally

Once EC governance is operative, founder authority alone cannot validly:

- withdraw Endowment principal;
- transfer or abandon `exergism.org` or persistent identifier authority;
- transfer organization-wide IP exclusively;
- merge, dissolve or legally succeed the institutional entity;
- grant patent rights on behalf of a rightsholder without express authority;
- amend contributor grants retroactively;
- distribute EC assets to Members; or
- amend the Mission Lock.

These actions require their applicable protected approval path even during the Founding Period.

## 8. Founding Period and power reduction

Founder authority should reduce as institutional capacity becomes real rather than according to a fictional calendar deadline.

### 8.1 Exact Founding Period boundary

For this framework, the **Founding Period** begins only when EC organization governance first becomes operative in `F0-founder-led-bootstrap` and includes both:

- `F0-founder-led-bootstrap`; and
- `F1-early-institution`.

The Founding Period ends only upon a valid, evidence-backed and recorded transition to `F2-distributed-institution` that satisfies every F2 transition criterion below and the machine-readable evidence gate in `policy/phase-evidence.json`.

Merely editing a phase string, reaching a calendar date, creating a legal entity, entering F1, or vacating the Founding Steward office does not end the Founding Period. If the Founding Steward dies, becomes incapacitated, resigns or is otherwise unavailable before a valid F2 transition, the applicable successor Mission Guardian may exercise only the mission-protection function provided by the adopted succession mechanism; the Founding Period itself continues until the F2 gate is validly satisfied.

Consequently, the guardian-consent requirement for Mission-Locked Amendments applies throughout F0 and F1. After a valid transition to F2, any continuing guardian-concurrence requirement must come from the adopted post-Founding-Period succession/mission-protection rule rather than being inferred from founder status.

### Phase F0 — founder-led bootstrap

Expected characteristics:

- no operative legal entity, or fewer than 3 Active Members;
- material infrastructure still depends on the founder;
- governance records and independent controls are incomplete.

Founder executive authority is broad, subject to the prohibitions above.

### Phase F1 — early institution

Transition criteria include at least:

- legally competent EC entity identified;
- at least 3 Active Members other than purely nominal accounts;
- operative Member Registry;
- conflict and records process;
- at least one independent institutional role holder besides the Founding Steward; and
- documented treasury/domain/repository delegations.

At F1, ordinary institutional decisions move to the Member/role processes defined by adopted policy. Founder executive authority becomes residual: continuity, appointments explicitly reserved by policy and mission protection.

### Phase F2 — distributed institution

Transition criteria include at least:

- at least 7 Active Members with meaningful participation;
- at least 12 months of operative governance records measured from the recorded governance-operative date;
- no single individual holding unilateral operational control over Treasury, persistent-domain recovery and all canonical repositories simultaneously;
- independent audit/review capacity; and
- demonstrated ability to replace delegated role holders without founder intervention.

At F2, the Founding Steward no longer has general executive primacy. Any continuing mission-protection authority must be the narrower Mission Guardian function defined by the adopted succession mechanism.

### Transition integrity and evidence

No phase transition may be declared solely by changing a string or boolean in a machine-readable file.

An operative F1 or F2 transition requires:

1. the applicable institutional decision record;
2. the objective Active-Member minimum derived from the operative Member Registry;
3. non-null evidence references for each non-count criterion required by the target phase;
4. for F2, at least 12 months between the recorded governance-operative date and the proposed phase-effective date; and
5. successful validation of `policy/phase-evidence.json` against the canonical human criteria above.

The evidence registry is a projection and integrity gate, not a substitute for the underlying records. Unsupported booleans or a phase label cannot manufacture institutional maturity.

A phase may not be artificially prevented merely to preserve founder executive power. Once an objective criterion is defined by adopted policy, machine-readable records should expose whether it is satisfied.

## 9. Succession, incapacity and disappearance

EC must be capable of surviving its founder.

The operative framework must therefore define a succession path for death, incapacity, prolonged unavailability or voluntary retirement of the Founding Steward.

The successor mission-protection function should be narrower than founder-led bootstrap authority. It may eventually be held by a Mission Guardian or plural Guardian body, but it must not acquire ownership of EC through succession.

Emergency succession cannot be used to rewrite historical contributor rights, repurpose persistent identifiers or distribute institutional assets.

## 10. Relationship to Membership

The Founding Steward may also be a Member and cast the same one-person vote as another eligible Member when non-conflicted.

Founder constitutional powers are separate from that vote and must be recorded separately. This prevents a hidden weighted-vote model.

A Member does not become Founding Steward, Mission Guardian, Treasurer, Domain Custodian or repository maintainer merely through membership.

## 11. Anti-capture interpretation

The bootstrap design rejects two false choices:

- that founder control must be hidden in technical credentials to preserve direction; or
- that immediate unrestricted majoritarian control is required to avoid founder capture.

EC instead treats capture risk as something to make **difficult, visible, limited and reversible**.

Founder authority is explicit and bounded. Member authority grows as the institution can sustain it. The Mission Lock prevents a later majority from casually changing what EC fundamentally is. Economic self-dealing remains constrained throughout.
